...
public class ShoppingViewModel
{
  ...
  public void reset()
  {
    ...
    catalogMap.clear();
    model.getCatalogOfProducts().forEach((product) -> catalogMap
        .put(product.getId(), new ProductViewModel(product)));
  ...
  public void setSelectedCatalogProductProperty(
      ProductViewModel productViewModel)
  {
    selectedCatalogProductProperty.set(productViewModel);
  }

  public void setSelectedBasketProductProperty(
      ProductViewModel productViewModel)
  {
  ...
  {
    ProductViewModel selectedCatalogProductViewModel = selectedCatalogProductProperty
        .get();
    if (selectedCatalogProductViewModel == null)
    {
      errorProperty.set(
          "!Please select a product from the catalog to be added to the cart.");
      return;
    }
    String selectedCatalogProductId = selectedCatalogProductViewModel
        .getIdProperty().getValue();
    if (basketMap.get(selectedCatalogProductId) == null)
    {
      basketMap.put(selectedCatalogProductId, new ProductViewModel(
          new Product(selectedCatalogProductId, 1,
              selectedCatalogProductViewModel.getNameProperty().getValue(),
              selectedCatalogProductViewModel.getDescriptionProperty()
                  .getValue(),
  ...
  {
    ProductViewModel selectedBasketProductViewModel = selectedBasketProductProperty
        .get();
    if (selectedBasketProductViewModel == null)
    {
      errorProperty
          .set("!Please select a product from the basket to be removed.");
      return false;
    }
    selectedBasketProductProperty.set(null);
      ...
      int newValue)
  {
    ProductViewModel selectedBasketProductViewModel = selectedBasketProductProperty
        .get();
    selectedBasketProductViewModel.getQuantityProperty().setValue(newValue);
  }
  ...
  private void updatePrices()
  {
    ...
    basketMap.forEach((key, value) -> preliminaryPrice[0] +=
        value.getPriceProperty().getValue() * value.getQuantityProperty()
            .getValue());
    priceProperty.set(String.format("%.2f", preliminaryPrice[0]));
    int discount = discountProperty.getValue();
    finalPriceProperty.set(String.format("%.2f", discount == 0 ?
        preliminaryPrice[0] :
        preliminaryPrice[0] * discount / 100));
  }
}
