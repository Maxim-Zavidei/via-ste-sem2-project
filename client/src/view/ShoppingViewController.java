...
public class ShoppingViewController extends ViewController
{
  ...
  @Override protected void init()
  {
    ...
    catalogAvailabilityColumn.setCellValueFactory(
        cellData -> cellData.getValue().getQuantityProperty());
    catalogNameColumn
        .setCellValueFactory(cellData -> cellData.getValue().getNameProperty());
    catalogDescriptionColumn.setCellValueFactory(
        cellData -> cellData.getValue().getDescriptionProperty());
    catalogPriceColumn.setCellValueFactory(
        cellData -> cellData.getValue().getPriceProperty());
    viewModel.getCatalogMap().addListener(
        (MapChangeListener.Change<? extends String, ? extends ProductViewModel> change) -> {
    ...
    catalogTable.getSelectionModel().selectedItemProperty().addListener(
        (obs, oldVal, newVal) -> viewModel
            .setSelectedCatalogProductProperty(newVal));

    // Bindings for the basket table.
    basketQuantityColumn.setCellValueFactory(
        cellData -> cellData.getValue().getQuantityProperty());
    basketNameColumn
        .setCellValueFactory(cellData -> cellData.getValue().getNameProperty());
    basketPriceColumn.setCellValueFactory(
        cellData -> cellData.getValue().getPriceProperty());
    basketCostColumn.setCellValueFactory(cellData -> new SimpleObjectProperty<>(
        cellData.getValue().getQuantityProperty().getValue() * cellData
            .getValue().getPriceProperty().getValue()));
    viewModel.getBasketMap().addListener(
        (MapChangeListener.Change<? extends String, ? extends ProductViewModel> change) -> {
          if (change.wasRemoved() ^ change.wasAdded())
            if (change.wasRemoved())
            {
    ...
    basketTable.getSelectionModel().selectedItemProperty().addListener(
        (obs, oldVal, newVal) -> viewModel
            .setSelectedBasketProductProperty(newVal));

    // Bindings for the rest of the user interface elements.
    errorLabel.textProperty().bind(viewModel.getErrorProperty());
    priceLabel.textProperty().bind(viewModel.getPriceProperty());
    Bindings.bindBidirectional(discountLabel.textProperty(),
        viewModel.getDiscountProperty(), new IntegerToString());
    Bindings.bindBidirectional(inputCouponField.textProperty(),
        viewModel.getInputCouponProperty());
    ...
    viewModel.getErrorProperty().addListener((obs, oldVal, newVal) -> errorLabel
        .setStyle("-fx-text-fill:" + (newVal.charAt(0) == '!' ?
            "#FF0000" :
            "#000000")));
    ...
    basketQuantityColumn.setCellFactory(
        TextFieldTableCell.forTableColumn(new IntegerStringConverter()));
    basketQuantityColumn.setOnEditCommit(
        event -> onQuantityEdit(event.getNewValue()));
  }
  ...
  @FXML private void dropFromBasket()
  {
    if (viewModel.dropFromBasket())
      basketTable.getSelectionModel().clearSelection();
  }
  ...
  private void onQuantityEdit(int newValue)
  {
    viewModel.onQuantityEdit(newValue);
  }
